@extends('layouts.app')
@section('title', 'Modifier une ' . $name)
@section('content')
        ...
        <div class="main-container container-fluid">
            <div class="inner-body" data-select2-id="18">
                ...
                <div class="page-header">
                    <div>
                        <h2 class="main-content-title tx-24 mg-b-5">Modifier une {{ $name }}</h2>
                        <ol class="breadcrumb">
                    ...
                    <div class="col-lg-12 col-md-12" data-select2-id="16">
                        <div class="card custom-card" data-select2-id="15">
                            <div class="card-header">
                                <div>
                                    <h6 class="main-content-label tx-15">Modifier une {{ $name }}</h6>
                                </div>
                            </div>
                            <div class="card-body" data-select2-id="14">
                                {{-- needs-validation was-validated --}}
                                <form class="row g-3" action="{{ route('formation-de-sécurité.update', $document->id) }}" novalidate=""
                                        ...
                                        <div class="col-lg-12 col-md-12">
                                            <div class="card custom-card">
                                                <div class="card-body">
                                                    ...
                                                    <label for="user_id" class="form-label tx-semibold">Sélectionnez un
                                                        utilisateur</label>
                                                    <select name="user_id" class="form-control notranslate mb-4"
                                                        id="user_id" required>
                                                        @foreach ($users as $user)
                                                            <option value="{{ $user->id }}"
                                                                @selected(old('user_id', @$document->user_id) == $user->id)>
                                                                {{ $user->name . ' (' . $user->role . ')' }}</option>
                                                        @endforeach
                                                    </select>
                                                    <label for="file"
                                                        class="form-label tx-semibold">{{ $name }}</label>
